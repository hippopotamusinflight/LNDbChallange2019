# arch3_20x20x20_final_v2

cube_dir = '/global/home/hpc4535/LNDbChallenge/LNDb_preprocessing_generate_all_cubes/cubes_for_CNN/all_cubes_20x20x20/'
# cube_dir = '/global/home/hpc4535/LNDbChallenge/LNDb_preprocessing_generate_all_cubes/cubes_for_CNN_testing/all_cubes_20x20x20/'
...
# ------------------------------- #
# preprocessing all cubes of 20x20x20
...
all_images_arr = np.array(all_images).reshape(-1,20,20,20,1)
...
x = tf.placeholder(tf.float32, [None,20,20,20,1], name="x_input")    # input placeholder
...
weights = {
    ...
    # only 1 max_pool ksize=2, stride=2, goes from 20x20x20 --> 10x10x10
    'wd1': tf.get_variable('W3', shape=(10*10*10*128,250), initializer=tf.contrib.layers.xavier_initializer()),     
...
#            opt = sess.run(train_step, feed_dict={x: batch_x, y: batch_y, keep_prob:0.8}) #, global_step:i*(len(train_imgs)//batch_size)+batch})     # what does global_step do?
...
#        print("job took: %2d minutes and %4.2f seconds\n" % ((epoch_end-epoch_start)//60, (epoch_end-epoch_start)%60))
    ...
    saved_path = saver.save(sess, '../output/arch3_20s_Adam_LR0_00001_BS128_c3_128filters_xavierInitializer_500epochs')      # removed global_step=global_step
